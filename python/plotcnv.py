...
RES = 10000
SKIP = 1000
# file="/home/jiangchen/ConvLFR/test_ConvLFR/WYQ_part/WYQ_part_testsort.bam"
def parse(sam):
    for chromo, length in zip(sam.references, sam.lengths):
        ...
        yield chromo, length, df.rolling(RES).mean()

if __name__ == "__main__":
    if len(sys.argv) <= 1:
        print("usage: python %s input.bam" % sys.argv[0], file=sys.stderr)
        sys.exit(1)
